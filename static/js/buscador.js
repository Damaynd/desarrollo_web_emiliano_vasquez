...
async function enviarNota(actividadId, nota, notaValor, notaContador, mensaje, boton) {
    ...
    if (!Number.isInteger(nota) || nota < 1 || nota > 7) {
        ...
        mostrarMensajeEvaluacion(mensaje, "Seleccione una nota valida entre 1 y 7.", true);
        return;
...
function crearResultadoActividad(actividad, patron) {
    ...
    articulo.appendChild(crearParrafo("Dia", actividad.dia));
    ...
    articulo.appendChild(crearParrafoDestacado("Descripcion", actividad.descripcion, patron));
...
function renderizarResultados(actividades, patron) {
    ...
    if (actividades.length === 0) {
        ...
        mostrarMensajeBusqueda("No se encontraron actividades para esa busqueda.");
        return;

    }

    mostrarMensajeBusqueda(`${actividades.length} actividades encontradas.`);
...
async function buscarActividades() {
    ...
    mostrarMensajeBusqueda("Buscando actividades...");
    ...
    try {
        ...
        if (!respuesta.ok) {
            ...
            throw new Error(datos.error || "No se pudo realizar la busqueda.");
